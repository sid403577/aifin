...
def buildMarketdata(stock:str,market:int):
    ...
    csv_file = open(f"/data/comments_data_{stock}_{market}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.csv", 'a+',
    ...
    while True:
        ...
        url = f"https://b2b-news.10jqka.com.cn/hxcota/market/stocks/v2/list?stock={stock}&market={market}&type=1&accessKey=74f9abd518ab0970&page={page}&pageSize=10"
        print(f"url:{url}")
                            ...
                            if len(id)>0:
                                ...
                                if response_detail.status_code==200:
                                    ...
                                    if 'data' in response_detail_text:
                                        ...
                                        s_date = time
                                        try:
                                            s_date = datetime.datetime.fromtimestamp(int(time)).strftime('%Y-%m-%d %H:%M:%S')
                                        except:
                                            print("发布时间解析异常，")
                                            s_date = time
                                        createTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                                        # 写入csv文件
                                        result_item1 = [s_date, title, content]
...
def store(docs: list[Document]):
    ...
    while True and count<3:
        try:
            Milvus(collection_name="tonghuashun").from_documents(
                docs,
                embeddings,
