...
llm_model_dict = {
    ...
    "fastchat-chatglm2-6b": {
        ...
        "local_model_path": None,
        "provides": "FastChatOpenAILLM",  # 使用fastchat api时，需保证"provides"为"FastChatOpenAILLM"
        "api_base_url": "http://43.163.245.99:8000/v1"  # "name"修改为fastchat服务中的"api_base_url"
    },
...
根据上述已知信息，以专业金融分析师的角度简洁和专业的来回答用户的问题，不考虑投资风险。使用符号[序号]表示引用的参考信息（已知信息的开头都有此序号），引用应放在每句话末尾。如果无法从中得到答案，请说 “根据已知信息无法回答该问题” 或 “没有提供足够的相关信息”，不允许在答案中添加编造成分，答案请使用中文。 
问题是：{question}"""
